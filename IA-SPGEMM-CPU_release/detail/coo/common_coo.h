...
{
    double coo_size=0;
    coo_size+=(sizeof(int))*(A_coo->row+1+2*A_coo->nnz+3);
    coo_size+=(sizeof(VALUE_TYPE))*(A_coo->nnz);
    return coo_size;
}
...
void CSRtoCOO(CsrMatrix* A_csr, CooMatrix *A_coo)
{
    ...
    A_coo->choice=true;
    if( sizeofcoo(A_coo)<(50*sizeofcsr(A_csr)))
    {
    ...
    A_coo->values=(VALUE_TYPE*)malloc1d(A_csr->nnz,sizeof(VALUE_TYPE));
...
void COO_MUL_COO(CooMatrix* A, CooMatrix* B, CooMatrix* C)
{
    ...
    C->values=(VALUE_TYPE*)malloc1d(C->nnz,sizeof(VALUE_TYPE));
