...
double sizeofdia(DiaMatrix *A_dia)
{
    ...
    dia_size+=(sizeof(double))*(A_dia->row*A_dia->num_diagonals);
    return dia_size;
}
...
void CSRtoDIA(CsrMatrix *A, DiaMatrix *A_dia)
{
    ...
    if( sizeofdia(A_dia)<(20*sizeofcsr(A)) )
    {
    ...
    A_dia->values=(double*)malloc1d(A_dia->row*A_dia->num_diagonals,sizeof(double));
    ...
    for(int i = 0; i < A->row; i++)
    {
        for(int jj = A->row_ind[i]; jj < A->row_ind[i+1]; jj++)
        {
            ...
            A_dia->values[i*A_dia->num_diagonals+diag] = A->values[jj];
        }
    }
...
void DIA_mul_DIA_phase1(DiaMatrix *A_dia, DiaMatrix *B_dia, DiaMatrix *C_dia)
{
    ...
    C_dia->values=(double*)malloc1d(C_dia->row*C_dia->num_diagonals,sizeof(double));
...
void DIA_mul_DIA_phase2(DiaMatrix *A_dia, DiaMatrix *B_dia, DiaMatrix *C_dia)
{
            ...
            if( A_j>=0 && A_j<A_dia->col )
            {
                ...
                for(int k=0;k<B_dia->num_diagonals;k++)
                {
                    ...
                    if( B_j>=0 && B_j<B_dia->col )
                    {
                        ...
                        //printf("C:%d-%d\n",out_i,out_j);
                        C_dia->values[out_i*C_dia->num_diagonals+out_j]+=A_dia->values[i*A_dia->num_diagonals+j]*B_dia->values[B_i*B_dia->num_diagonals+k];
...
void print_dia(DiaMatrix *A_dia)
{
    ...
    {
        for(int j=0;j<A_dia->num_diagonals;j++)
            printf("%5.2lf,",A_dia->values[i*A_dia->num_diagonals+j]);
        printf("\n");
    }
